...
def test_validate_integer_type(sample_df):
    """Test integer type validation with explicit type coercion to ensure robustness."""
    validator = DataValidator()
    validator.add_rule(ValidationRule("age", DataType.INTEGER))

    # First test with clean data
    result = validator.validate_dataframe(sample_df)
    assert result.is_valid is True

    # Introduce bad data - make column object dtype first (more realistic test)
    # This ensures the validator handles mixed/invalid types properly
    bad_df = sample_df.copy()
    
    # Step 1: Force column to object dtype (simulates real-world dirty data)
    bad_df["age"] = bad_df["age"].astype(object)
    
    # Step 2: Assign invalid string value
